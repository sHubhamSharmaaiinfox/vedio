import { ITrackItemsMap, useEditorState } from '@designcombo/core';

type ItemType = "text" | "image" | "video" | "audio" | "search" | "type" | "media" | "add" | "arrowRight" | "arrowUpRight" | "billing" | "bookOpen" | "chevronLeft" | "chevronRight" | "check" | "warning" | "helper";
...
export default function ControlList() {
  const { activeIds, trackItemsMap } = useEditorState<{ activeIds: string[]; trackItemsMap: ITrackItemsMap }>((state) => ({
    ...
    >
      {
        ...
        }[controlType as keyof typeof controlType]
      }
    </div>
  );
}
...
const TextMenuList = ({
  ...
  openToolboxItem: (type: string) => void;
  type: "text" | "image" | "video" | "audio" | "search" | "type" | "media" | "add" | "arrowRight" | "arrowUpRight" | "billing" | "bookOpen" | "chevronLeft" | "chevronRight" | "check" | "warning";
}) => {
  return (
    <div className="flex flex-col items-center">
      {["text", "image", "video", "audio"].includes(type) && (
        <PresetsMenuListItem type={type as "text" | "image" | "video" | "audio"} openToolboxItem={openToolboxItem} />
      )}
